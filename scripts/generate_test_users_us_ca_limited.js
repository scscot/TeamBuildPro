...
function getRandomCountryAndState() {
  const country = faker.helpers.arrayElement(["United States", "Canada"]);
  const state = country === "United States"
    ? faker.helpers.arrayElement(usStates)
    : faker.helpers.arrayElement(caProvinces);
...
function createUser(level, referredBy, uplineAdminUid, adminProfile) {
  ...
  const { country, state } = getRandomCountryAndState();
  return {
    uid,
