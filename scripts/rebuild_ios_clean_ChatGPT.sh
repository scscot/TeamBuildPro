...
FLUTTER_FRAMEWORK_PATH=build/ios/iphoneos/Flutter/Flutter.framework
if [ -d "$FLUTTER_FRAMEWORK_PATH" ]; then
  codesign --force --sign "Apple Development" --timestamp=none "$FLUTTER_FRAMEWORK_PATH/Flutter"
  echo "✅ Flutter.framework re-signed manually."
else
  echo "⚠️ Flutter.framework not found, skipping manual sign."
fi
 ---
