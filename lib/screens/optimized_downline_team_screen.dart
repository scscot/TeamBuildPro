...
class OptimizedDownlineTeamScreen extends StatefulWidget {
  final String referredBy;
  const OptimizedDownlineTeamScreen({super.key, required this.referredBy});

  @override
  State<OptimizedDownlineTeamScreen> createState() => _OptimizedDownlineTeamScreenState();
}

class _OptimizedDownlineTeamScreenState extends State<OptimizedDownlineTeamScreen> {
  bool isLoading = true;
  ...
  Timer? _debounce;
  int levelOffset = 0;
  List<UserModel> _downlineUsers = []; // Stores only relevant downline users fetched from downlineIds
  ...
  Future<void> fetchDownline() async {
    ...
    try {
      ...
      if (currentUserId == null) {
        debugPrint('⚠️ Current user UID is null. Cannot fetch downline.');
        setState(() => isLoading = false);
        return;
      }

      // 1. Fetch current user's document to get their pre-computed downlineIds
      final currentUserDoc = await FirebaseFirestore.instance.collection('users').doc(currentUserId).get();
      if (!currentUserDoc.exists) {
        debugPrint('⚠️ Current user document not found. Cannot fetch downline.');
        setState(() => isLoading = false);
        return;
      }

      final currentUserData = currentUserDoc.data();
      // Ensure downlineIds is treated as List<String>
      final List<String> downlineUids = List<String>.from(currentUserData?['downlineIds'] ?? []);
      levelOffset = currentUserData?['level'] ?? 0;
      uplineBizOpp = currentUserData?['biz_opp']; // Assuming biz_opp is directly on the user doc or admin settings

      // Handle the case where downlineIds is empty
      if (downlineUids.isEmpty) {
        setState(() {
          _downlineUsers = [];
          downlineByLevel = {};
        ...
        debugPrint('✅ No downline IDs found for current user.');
        return;
      }

      // 2. Fetch only the user documents specified in downlineUids
      //    Handle batching for 'whereIn' queries (max 10 elements per query)
      List<UserModel> fetchedDownlineUsers = [];
      const int batchSize = 10; // Firestore limit for 'whereIn' clause

      for (int i = 0; i < downlineUids.length; i += batchSize) {
        final end = (i + batchSize < downlineUids.length) ? i + batchSize : downlineUids.length;
        final subList = downlineUids.sublist(i, end);
        if (subList.isNotEmpty) {
          final querySnapshot = await FirebaseFirestore.instance
              .collection('users')
              .where(FieldPath.documentId, whereIn: subList)
              .get();
          fetchedDownlineUsers.addAll(querySnapshot.docs.map((doc) {
            final data = doc.data();
            return UserModel(
              uid: doc.id,
              email: data['email'] ?? '',
      ...
      _downlineUsers = fetchedDownlineUsers; // Update the list of downline users for display
      ...
      // Now, iterate through the *fetchedDownlineUsers* (which is _downlineUsers)
      for (var user in _downlineUsers) {
